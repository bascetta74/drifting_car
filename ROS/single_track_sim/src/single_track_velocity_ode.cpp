...
single_track_velocity_ode::single_track_velocity_ode(double deltaT, tyreModel tyre_model, actuatorModel actuator_model) : dt(deltaT),
...
void single_track_velocity_ode::setInitialState(double r0, double Vy0, double x0, double y0, double psi0)
{
    // Initial state values
    state[0] = r0;
    state[1] = Vy0;
...
void single_track_velocity_ode::setVehicleParams(double m, double a, double b, double Cf, double Cr, double mu, double Iz)
{
...
void single_track_velocity_ode::setSteeringActuatorParams(double gain, double frequency, double damping, int delay)
{
...
void single_track_velocity_ode::setVelocityActuatorParams(double gain)
{
    // Initialize velocity actuator parameters
    mu_speed = gain;
}

void single_track_velocity_ode::setReferenceCommands(double velocity, double steer)
{
    Vx_ref = velocity;
...
void single_track_velocity_ode::integrate()
{
    ...
    if ((!steeringActuatorParams_set) && (steeringActuator_model!=single_track_velocity_ode::IDEAL)) {
        throw std::invalid_argument( "Steering actuator parameters not set!" );
    }

    // Integrate for one step ahead
    using namespace std::placeholders;
    stepper.do_step(std::bind(&single_track_velocity_ode::vehicle_ode, this, _1, _2, _3), state, t, dt);
...
void single_track_velocity_ode::vehicle_ode(const state_type &state, state_type &dstate, double t)
{
