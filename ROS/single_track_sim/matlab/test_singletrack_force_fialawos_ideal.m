...
res = load('test_single_track_force_fialawos_ideal.txt');
ode_res.t         = res(:,1);
...
if (actuator_model==0)
    load_system('car_simulator_force');
    set_param('car_simulator_force/Actuator models','Commented','through');
else
    load_system('car_simulator_force');
    set_param('car_simulator_force/Actuator models','Commented','off');
end

sim_res = sim('car_simulator_force.slx','StartTime','steer_ref.Time(1)','StopTime',...
