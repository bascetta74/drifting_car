...
% Run cpp simulation
system('../test/test_singletrack_vxvy_velocity_ode fiala_wo_sat ideal');

% Load cpp simulation results
res = load('test_singletrack_vxvy_velocity_fiala_wo_sat_ideal.txt');
ode_res.t         = res(:,1);
...
delete('test_singletrack_vxvy_velocity_fiala_wo_sat_ideal.txt');
...
if (actuator_model==0)
    load_system('carsim_vxvy_velocity');
    set_param('carsim_vxvy_velocity/Actuator models','Commented','through');
else
    load_system('carsim_vxvy_velocity');
    set_param('carsim_vxvy_velocity/Actuator models','Commented','off');
end
