...
% Run cpp simulation
system('../test/test_singletrack_beta_force_ode fiala_wo_sat ideal');

% Load cpp simulation results
res = load('test_singletrack_beta_force_fiala_wo_sat_ideal.txt');
ode_res.t         = res(:,1);
...
delete('test_singletrack_beta_force_fiala_wo_sat_ideal.txt');
...
figure,
subplot(2,1,1),...
    plot(sim_res.time,sim_res.V.*cos(sim_res.beta), ode_res.t,ode_res.vx,'--'),ylabel('Vx [m/s]'),xlabel('Time (s)'),...
    grid,legend('Simulink','ODEINT')
subplot(2,1,2),...
    plot(ode_res.t,abs(sim_res.V.*cos(sim_res.beta)-ode_res.vx)),ylabel('Vx abs error [m/s]'),xlabel('Time (s)'),...
    grid

% Plot lateral dynamics measured outputs
figure,
subplot(2,1,1),...
    plot(sim_res.time,sim_res.V.*sin(sim_res.beta), ode_res.t,ode_res.vy,'--'),ylabel('Vy [m/s]'),xlabel('Time (s)'),...
    grid,legend('Simulink','ODEINT')
subplot(2,1,2),...
    plot(ode_res.t,abs(sim_res.V.*sin(sim_res.beta)-ode_res.vy)),ylabel('Vy abs error [m/s]'),xlabel('Time (s)'),...
    grid
