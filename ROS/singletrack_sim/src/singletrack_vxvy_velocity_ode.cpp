...
singletrack_vxvy_velocity_ode::singletrack_vxvy_velocity_ode(double deltaT, tyreModel tyre_model, actuatorModel actuator_model) : dt(deltaT),
...
void singletrack_vxvy_velocity_ode::setInitialState(double r0, double Vy0, double x0, double y0, double psi0)
{
    // Initial state values
    state[0] = r0;
    state[1] = Vy0;
...
void singletrack_vxvy_velocity_ode::setVehicleParams(double m, double a, double b, double Cf, double Cr, double mu, double Iz)
{
...
void singletrack_vxvy_velocity_ode::setSteeringActuatorParams(double gain, double frequency, double damping, int delay)
{
...
void singletrack_vxvy_velocity_ode::setVelocityActuatorParams(double gain)
{
...
void singletrack_vxvy_velocity_ode::integrate()
{
    ...
    if ((!steeringActuatorParams_set) && (steeringActuator_model!=singletrack_vxvy_velocity_ode::IDEAL)) {
        throw std::invalid_argument( "Steering actuator parameters not set!" );
    }

    // Integrate for one step ahead
    using namespace std::placeholders;
    stepper.do_step(std::bind(&singletrack_vxvy_velocity_ode::vehicle_ode, this, _1, _2, _3), state, t, dt);
...
void singletrack_vxvy_velocity_ode::vehicle_ode(const state_type &state, state_type &dstate, double t)
{
    ...
    dstate[1] = (Fyf*std::cos(delta)+Fyr)/m-r*Vx;   // dVy
    dstate[2] = std::cos(psi)*Vx-std::sin(psi)*Vy;  // dx
    dstate[3] = std::sin(psi)*Vx+std::cos(psi)*Vy;  // dy
    dstate[4] = r;                                  // r
