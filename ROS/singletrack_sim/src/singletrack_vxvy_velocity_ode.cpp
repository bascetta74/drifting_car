...
singletrack_vxvy_velocity_ode::singletrack_vxvy_velocity_ode(double deltaT, tyreModel tyre_model, actuatorModel actuator_model, double vx_thd) : dt(deltaT),
...
{
    // state = [ r, Vy, x, y, psi, steer_pos, steer_vel, speed_pos, speed_vel ]
...
void singletrack_vxvy_velocity_ode::integrate()
{
    ...
    // Check actuator parameters are set
    if ((!actuatorParams_set) && (actuator_model!=singletrack_vxvy_velocity_ode::IDEAL)) {
    ...
    if (actuator_model==singletrack_vxvy_velocity_ode::REAL) {
        delta_ref_FIFO.pop();
        Vx_ref_FIFO.pop();
    }
}
