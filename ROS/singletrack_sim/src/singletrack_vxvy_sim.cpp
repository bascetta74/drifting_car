...
void singletrack_vxvy_sim::Prepare(void) {
    ...
    vehicleCommand_subscriber = Handle.subscribe("/controller_cmd", 1,
                                                 &singletrack_vxvy_sim::vehicleCommand_MessageCallback, this);
    ...
    if (actuator_model == 0) {         // ideal
        if (tyre_model == 0) {         // linear
            if (input_cmd == 0) {      // velocity
                sim_velocity = new singletrack_vxvy_velocity_ode((double)DT_SIMULATOR*1.0E-6, singletrack_vxvy_velocity_ode::LINEAR,
                                                                 singletrack_vxvy_velocity_ode::IDEAL, 0.05);
                ROS_INFO("Node %s running vxvy_velocity model with linear tyres and ideal actuators.",
                         ros::this_node::getName().c_str());
            } else {                   // force
                sim_force = new singletrack_vxvy_force_ode((double)DT_SIMULATOR*1.0E-6, singletrack_vxvy_force_ode::LINEAR,
                                                           singletrack_vxvy_force_ode::IDEAL, 0.05);
                ROS_INFO("Node %s running vxvy_force model with linear tyres and ideal actuators.",
                         ros::this_node::getName().c_str());
            }
        } else if (tyre_model == 1) {  // fiala with saturation
            if (input_cmd == 0) {      // velocity
                sim_velocity = new singletrack_vxvy_velocity_ode((double)DT_SIMULATOR*1.0E-6,
                                                                 singletrack_vxvy_velocity_ode::FIALA_WITH_SATURATION,
                                                                 singletrack_vxvy_velocity_ode::IDEAL, 0.05);
                ROS_INFO("Node %s running vxvy_velocity model with Fiala with saturation tyres and ideal actuators.",
                         ros::this_node::getName().c_str());
            } else {                   // force
                sim_force = new singletrack_vxvy_force_ode((double)DT_SIMULATOR*1.0E-6, singletrack_vxvy_force_ode::FIALA_WITH_SATURATION,
                                                           singletrack_vxvy_force_ode::IDEAL, 0.05);
                ROS_INFO("Node %s running vxvy_force model with Fiala with saturation tyres and ideal actuators.",
                         ros::this_node::getName().c_str());
            }
        } else if (tyre_model == 2) {  // fiala witout saturation
            if (input_cmd == 0) {      // velocity
                sim_velocity = new singletrack_vxvy_velocity_ode((double)DT_SIMULATOR*1.0E-6,
                                                                 singletrack_vxvy_velocity_ode::FIALA_WITHOUT_SATURATION,
                                                                 singletrack_vxvy_velocity_ode::IDEAL, 0.05);
                ROS_INFO("Node %s running vxvy_velocity model with Fiala without saturation tyres and ideal actuators.",
                         ros::this_node::getName().c_str());
            } else {                   // force
                sim_force = new singletrack_vxvy_force_ode((double)DT_SIMULATOR*1.0E-6, singletrack_vxvy_force_ode::FIALA_WITHOUT_SATURATION,
                                                           singletrack_vxvy_force_ode::IDEAL, 0.05);
                ROS_INFO("Node %s running vxvy_force model with Fiala without saturation tyres and ideal actuators.",
                         ros::this_node::getName().c_str());
            }
        } else {                       // error
        ...
        if (tyre_model == 0) {         // linear
            if (input_cmd == 0) {      // velocity
                sim_velocity = new singletrack_vxvy_velocity_ode((double)DT_SIMULATOR*1.0E-6, singletrack_vxvy_velocity_ode::LINEAR,
                                                                 singletrack_vxvy_velocity_ode::REAL, 0.05);
                ROS_INFO("Node %s running vxvy_velocity model with linear tyres and real actuators.",
                         ros::this_node::getName().c_str());
            } else {                   // force
                sim_force = new singletrack_vxvy_force_ode((double)DT_SIMULATOR*1.0E-6, singletrack_vxvy_force_ode::LINEAR,
                                                           singletrack_vxvy_force_ode::REAL, 0.05);
                ROS_INFO("Node %s running vxvy_force model with linear tyres and real actuators.",
                         ros::this_node::getName().c_str());
            }
        } else if (tyre_model == 1) {  // fiala with saturation
            if (input_cmd == 0) {      // velocity
                sim_velocity = new singletrack_vxvy_velocity_ode((double)DT_SIMULATOR*1.0E-6,
                                                                 singletrack_vxvy_velocity_ode::FIALA_WITH_SATURATION,
                                                                 singletrack_vxvy_velocity_ode::REAL, 0.05);
                ROS_INFO("Node %s running vxvy_velocity model with Fiala with saturation tyres and real actuators.",
                         ros::this_node::getName().c_str());
            } else {                   // force
                sim_force = new singletrack_vxvy_force_ode((double)DT_SIMULATOR*1.0E-6, singletrack_vxvy_force_ode::FIALA_WITH_SATURATION,
                                                           singletrack_vxvy_force_ode::REAL, 0.05);
                ROS_INFO("Node %s running vxvy_force model with Fiala with saturation tyres and real actuators.",
                         ros::this_node::getName().c_str());
            }
        } else if (tyre_model == 2) {  // fiala witout saturation
            if (input_cmd == 0) {      // velocity
                sim_velocity = new singletrack_vxvy_velocity_ode((double)DT_SIMULATOR*1.0E-6,
                                                                 singletrack_vxvy_velocity_ode::FIALA_WITHOUT_SATURATION,
                                                                 singletrack_vxvy_velocity_ode::REAL, 0.05);
                ROS_INFO("Node %s running vxvy_velocity model with Fiala without saturation tyres and real actuators.",
                         ros::this_node::getName().c_str());
            } else {                   // force
                sim_force = new singletrack_vxvy_force_ode((double)DT_SIMULATOR*1.0E-6, singletrack_vxvy_force_ode::FIALA_WITHOUT_SATURATION,
                                                           singletrack_vxvy_force_ode::REAL, 0.05);
                ROS_INFO("Node %s running vxvy_force model with Fiala without saturation tyres and real actuators.",
                         ros::this_node::getName().c_str());
            }
        } else {                       // error
            ROS_ERROR("Node %s: unknown tyre model.", ros::this_node::getName().c_str());
