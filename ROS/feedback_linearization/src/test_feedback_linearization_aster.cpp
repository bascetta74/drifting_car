#include "feedback_linearization/test_feedback_linearization_aster.h"
...
void test_feedback_linearization_aster::Prepare(void)
{
 ...
 /* ROS topics */
 vehiclePose_subscriber = Handle.subscribe("/odom", 1, &test_feedback_linearization_aster::vehiclePose_MessageCallback, this);
 vehicleIMU_subscriber = Handle.subscribe("/ackermann/crio", 1, &test_feedback_linearization_aster::vehicleIMU_MessageCallback, this);
 startcmd_subscriber = Handle.subscribe("/start_cmd", 1, &test_feedback_linearization_aster::startCommand_MessageCallback, this);

 commandVel_publisher = Handle.advertise<geometry_msgs::Twist>("/cmd_vel", 1);
...
void test_feedback_linearization_aster::RunPeriodically(float Period)
{
...
void test_feedback_linearization_aster::Shutdown(void)
{
 if (_linearizer)
 {
   delete _linearizer;
...
void test_feedback_linearization_aster::vehiclePose_MessageCallback(const nav_msgs::Odometry::ConstPtr &msg)
{
  /* Updating position buffer */
  _vehiclePositionXBuffer.push_back(msg->pose.pose.position.x);
  _vehiclePositionYBuffer.push_back(msg->pose.pose.position.y);

  /* Updating 2D pose */
  _vehiclePose.at(0) = msg->pose.pose.position.x;
  _vehiclePose.at(1) = msg->pose.pose.position.y;
...
void test_feedback_linearization_aster::vehicleIMU_MessageCallback(const aster_msgs::Crio::ConstPtr& msg)
{
  /* Updating yaw rate */
  _vehicleAngularVelocity = msg->iry;
}

void test_feedback_linearization_aster::startCommand_MessageCallback(const std_msgs::Bool::ConstPtr& msg)
{
  _start_control = msg->data;
}

void test_feedback_linearization_aster::PeriodicTask(void)
{
  ...
  if (_start_control)
  {
    ...
    msg.linear.x = speed*cos(_vehicleSideslip);
    msg.linear.y = msg.linear.z = 0.0;
    msg.angular.x = msg.angular.y = 0.0;
    msg.angular.z = steer;
    commandVel_publisher.publish(msg);
  }
  else
  {
    _time = 0;
