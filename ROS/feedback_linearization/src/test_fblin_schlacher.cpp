...
void test_fblin_schlacher::Prepare(void) {
    ...
    /* ROS topics */
    vehiclePose_subscriber = Handle.subscribe("/car/ground_pose", 1, &test_fblin_schlacher::vehiclePose_MessageCallback,
                                              this);
    vehicleIMU_subscriber = Handle.subscribe("/imu/data", 1, &test_fblin_schlacher::vehicleIMU_MessageCallback, this);
    telemetry_subscriber = Handle.subscribe("/car_simulator/telemetry", 1,
                                            &test_fblin_schlacher::simulated_telemetry_MessageCallback, this);
    ...
    controllerState_publisher = Handle.advertise<std_msgs::Float64MultiArray>("/feedback_linearization/controllerState",
                                                                              1);
    ...
    if (use_sim_time) {
        if (use_ideal_sim) {
            ROS_INFO("Node %s running in simulation mode with simulated measurements.",
...
void test_fblin_schlacher::vehiclePose_MessageCallback(const geometry_msgs::Pose2D::ConstPtr &msg) {
    if (!use_ideal_sim) {
        ...
        if ((_car_control_state == car_msgs::car_cmd::STATE_AUTOMATIC) ||
            (_car_control_state == car_msgs::car_cmd::STATE_MANUAL)) {
...
void test_fblin_schlacher::PeriodicTask(void) {
    ...
    if (_car_control_state == car_msgs::car_cmd::STATE_AUTOMATIC) {
        ...
        if (_time <= T0) {
            ...
            z1ref = _vx0 +
                    (3 * std::pow(_time - T0, 2.0) * T - 2 * std::pow(_time - T0, 3.0)) / std::pow(T, 3.0) * delta_vx;
        } else {
        ...
        if (_time <= T1 + T0) {
            ...
            z2ref = -A1 * std::pow(_time - T0 - T1, 3.0) * std::pow((T2 - T1) - (_time - T0 - T1), 3.0) /
                    std::pow(T2 - T1, 6.0);
        } else if (_time <= T3 + T0) {
            z2ref = -A2 * std::pow(_time - T0 - T2, 3.0) * std::pow((T3 - T2) - (_time - T0 - T2), 3.0) /
                    std::pow(T3 - T2, 6.0);
        } else {
            z2ref = 0;
        }
        ...
        if (_linearizer) {
            ...
            _linearizer->set_bicycleState(
                    std::sqrt(std::pow(_vehicleVelocity.at(0), 2.0) + std::pow(_vehicleVelocity.at(1), 2.0)),
                    _vehicleSideslip, _vehicleAngularVelocity);
