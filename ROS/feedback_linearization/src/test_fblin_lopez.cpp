...
void test_fblin_lopez::Prepare(void) {
    ...
    /* ROS topics */
    vehiclePose_subscriber = Handle.subscribe("/car/ground_pose", 1, &test_fblin_lopez::vehiclePose_MessageCallback,
                                              this);
    ...
    telemetry_subscriber = Handle.subscribe("/car_simulator/telemetry", 1,
                                            &test_fblin_lopez::simulated_telemetry_MessageCallback, this);
    ...
    pointPvelocity_publisher = Handle.advertise<geometry_msgs::TwistStamped>("/feedback_linearization/pointP_actVel",
                                                                             1);
...
void test_fblin_lopez::PeriodicTask(void) {
    ...
    if (_car_control_state == car_msgs::car_cmd::STATE_AUTOMATIC) {
        ...
        if (_linearizer) {
            _linearizer->set_bicycleState(_vehiclePose.at(0), _vehiclePose.at(1), _vehiclePose.at(2), _vehicleSideslip,
                                          _vehicleAngularVelocity);
        } else
            ROS_ERROR("Error, no feedback linearization has been activated");
