...
void AGS_AFI_controller::Prepare(void) {
    ...
    if (_AGS_controller) {
        ROS_INFO("Node %s: controller data loaded from %s.", ros::this_node::getName().c_str(),
...
void AGS_AFI_controller::vehiclePose_MessageCallback(const geometry_msgs::Pose2D::ConstPtr &msg) {
    ...
    if (_car_control_state == car_msgs::car_cmd::STATE_AUTOMATIC) {
        ...
        _vehicleLongitudinalVelocity =
                sqrt(pow(_vehicleVelocity.at(0), 2) + pow(_vehicleVelocity.at(1), 2)) * cos(_vehicleSideslip);
    } else {
    ...
    if (_car_control_state == car_msgs::car_cmd::STATE_AUTOMATIC) {
        ...
        if (_AGS_controller) {
            if (_vehicleLongitudinalVelocity > speed_thd) {
                ...
                Eigen::Vector3d AGS_param(1.0 / _vehicleLongitudinalVelocity,
                                          1.0 / std::pow(_vehicleLongitudinalVelocity, 2.0), _dampingRef);
