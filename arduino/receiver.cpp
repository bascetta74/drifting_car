...
volatile receiver_channel receiver_steer, receiver_speed, receiver_button;
...
{
	return receiver_steer.state;
}

byte get_speed_state_ms()
{
	return receiver_speed.state;
}
...
ISR (PCINT1_vect)
{
	...
	if (PINC & B00000001) {
		...
		if (receiver_steer.state == 0) {
			// Update channel state
			receiver_steer.state = 1;
			receiver_steer.edge_time = receiver_current_time;
		}
	}
	// Input A0 changed from HIGH to LOW
	else if (receiver_steer.state == 1) {
		// Update channel state
		receiver_steer.state = 0;
	...
	if (PINC & B00000010) {
		...
		if (receiver_speed.state == 0) {
			// Update channel state
			receiver_speed.state = 1;
			receiver_speed.edge_time = receiver_current_time;
		}
	}
	// Input A1 changed from HIGH to LOW
	else if (receiver_speed.state == 1) {
		// Update channel state
		receiver_speed.state = 0;
	...
	if (PINC & B00000100) {
		...
		if (receiver_button.state == 0) {
			// Update channel state
			receiver_button.state = 1;
